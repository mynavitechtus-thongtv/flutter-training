...
- [All lint rules](#all-lint-rules)
  ...
  - [37. missing_test_group](#37-missing_test_group)
...
Test file phải có đủ một `group()` cho mỗi constructor (default + factory/named) của file source tương ứng. Lint đếm số constructor trong file lib và yêu cầu file test có đúng số group với tên trùng (tên class cho default, tên factory cho factory).

**Parameters**: [common parameters](#common-parameters) + `test_folder_prefix` (default: `"test/widget_test"`)

**QuickFix**: Không có

```yaml
- missing_test_group:
  test_folder_prefix: "test/widget_test"
```

**Good**:

```dart
// confirm_dialog_test.dart (source: ConfirmDialog + deleteAccount, logOut)
void main() {
  group('ConfirmDialog', () { ... });
  group('deleteAccount', () { ... });
  group('logOut', () { ... });
...
```dart
// confirm_dialog_test.dart - missing groups
void main() {
  group('ConfirmDialog', () { ... });
  // missing group('deleteAccount') and group('logOut')
}
```
